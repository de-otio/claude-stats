...
| [cost-correctness-2026-09/](cost-correctness-2026-09/) | Delta on the two schema-drift sets after eleven days of new data and Anthropic's new cost tooling. Headline: **Claude Stats over-reports cost by 2.34×** — the known `message.id` dedupe defect (1.94×, reproduced on a third sample) compounding with a new one — point-release model ids prefix-matching onto their predecessor's rate row, which prices `claude-opus-4-7` from the **retired** Opus 4 row (**3×**, $26,923 in the local database — the release's largest correction) and `claude-fable-5-1` from Fable 5's (**4×** on cache reads). Retracts the prior set's instruction to bump the Claude Sonnet 5 row (live pricing says the increase never happened; the edit would have introduced a 50% over-report), corroborates its `[1m]` reversal, and scopes a correctness-only release |
