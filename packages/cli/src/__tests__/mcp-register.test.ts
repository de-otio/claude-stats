...
vi.mock("vscode", () => ({
  ...
  workspace: {
    // Default: return the schema default ("auto") for any get() call.
    // Individual tests may override the inspector mock to return "off".
...
describe("ensureMcpServer", () => {
  ...
  it("writes CLAUDE_STATS_DEFAULT_EMBEDDINGS=auto into env when setting defaults to 'auto'", () => {
    ensureMcpServer(mockContext);

    const json = readClaudeJson();
    const servers = json.mcpServers as Record<string, { env?: Record<string, string> }>;
    expect(servers["claude-stats"]!.env).toBeDefined();
    expect(servers["claude-stats"]!.env!["CLAUDE_STATS_DEFAULT_EMBEDDINGS"]).toBe("auto");
  });

  it("writes CLAUDE_STATS_DEFAULT_EMBEDDINGS=off when setting is 'off'", () => {
    vi.mocked(vscode.workspace.getConfiguration).mockReturnValueOnce({
      get: <T,>(_key: string, _default?: T): unknown => "off",
    ...
    const json = readClaudeJson();
    const servers = json.mcpServers as Record<string, { env?: Record<string, string> }>;
    expect(servers["claude-stats"]!.env!["CLAUDE_STATS_DEFAULT_EMBEDDINGS"]).toBe("off");
  });

  it("re-writes ~/.claude.json when only the env field changes (settings update)", () => {
    // First call: setting = auto → writes auto
    ensureMcpServer(mockContext);
    let json = readClaudeJson();
    let servers = json.mcpServers as Record<string, { env: Record<string, string> }>;
    expect(servers["claude-stats"]!.env["CLAUDE_STATS_DEFAULT_EMBEDDINGS"]).toBe("auto");

    // Second call with different setting must update env, not skip as up-to-date.
    vi.mocked(vscode.workspace.getConfiguration).mockReturnValueOnce({
      get: <T,>(_key: string, _default?: T): unknown => "off",
    } as unknown as ReturnType<typeof vscode.workspace.getConfiguration>);
    ...
    ensureMcpServer(mockContext);
    json = readClaudeJson();
    servers = json.mcpServers as Record<string, { env: Record<string, string> }>;
    expect(servers["claude-stats"]!.env["CLAUDE_STATS_DEFAULT_EMBEDDINGS"]).toBe("off");
  });
