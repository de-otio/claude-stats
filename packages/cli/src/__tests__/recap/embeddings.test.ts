...
// ─── createEmbeddingProvider — bundled VSIX layout ───────────────────────────

describe('createEmbeddingProvider — bundled layout (VSIX)', () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'emb-bundled-'));
  });
  ...
  it('hash mismatch on bundled ONNX → returns null without deleting (read-only VSIX)', async () => {
    const modelDir = path.join(tmpDir, 'embed-model');
    const cacheDir = path.join(tmpDir, 'cache');
    ...
    writeDummy(bundledPath, 'tampered bundled bytes');
    ...
    expect(provider).toBeNull();
    // VSIX is read-only — file MUST NOT be deleted
    expect(fs.existsSync(bundledPath)).toBe(true);
  });

  it('mode=off ignores bundled layout entirely', async () => {
    const modelDir = path.join(tmpDir, 'embed-model');
    const bundledPath = path.join(
      modelDir,
      'Xenova',
    ...
    // Even a corrupted bundle should not be checked when mode=off
    writeDummy(bundledPath);
    ...
    expect(provider).toBeNull();
    // No deletion, no hash check, no logging side effects
    expect(fs.existsSync(bundledPath)).toBe(true);
  });

  it('falls through to legacy single-file layout when bundled file is absent', async () => {
    const modelDir = path.join(tmpDir, 'embed-model');
    fs.mkdirSync(modelDir, { recursive: true });
    // No bundled file, no legacy file → mode=auto returns null cleanly
