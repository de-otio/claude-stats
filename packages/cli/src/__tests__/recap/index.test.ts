...
describe('buildDailyDigest — single session, no git', () => {
  ...
  it('reports clusteringMethod = "jaccard" when no embeddingProvider is supplied', async () => {
    const digest = await buildDailyDigest(
      store,
      { date: '2024-01-15', tz: 'UTC' },
      defaultDeps({ getProjectGitActivity: vi.fn(() => null) }),
  ...
  it('reports clusteringMethod = "embeddings" when an embeddingProvider is supplied', async () => {
    const stubProvider = {
      embed: async (_: string) => new Float32Array([1, 0, 0]),
      cosine: (_a: Float32Array, _b: Float32Array) => 1,
    };
    const digest = await buildDailyDigest(
      store,
      { date: '2024-01-15', tz: 'UTC' },
      defaultDeps({
        getProjectGitActivity: vi.fn(() => null),
        embeddingProvider: stubProvider,
      }),
    );
    expect(digest.clusteringMethod).toBe('embeddings');
  });
});
