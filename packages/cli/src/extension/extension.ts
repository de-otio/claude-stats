...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // Verify the bundled embedding model's SHA-256 in the background. A
  // mismatch implies marketplace or local-FS tamper — surface a warning
  // notification once, then let the MCP runtime's existing fail-closed
  // behaviour drop the digest back to lexical clustering.
  void verifyBundledModel(context.extensionPath).then((result) => {
    if (result.ok) return;
    if (result.reason === "missing") {
      // A correctly-built VSIX always ships the bundled model. Missing
      // here means a packaging bug, not a security event — log only.
      console.warn(
        `[claude-stats] bundled embedding model not found at ${result.modelPath}; ` +
          "embeddings unavailable, falling back to lexical clustering",
      );
      return;
    }
    // hash-mismatch
    console.warn(
      `[claude-stats] bundled embedding model integrity check failed at ${result.modelPath}; ` +
        `actual sha256: ${result.actual}; falling back to lexical clustering`,
    );
    void vscode.window.showWarningMessage(t("extension:embeddings.tampered"));
  });

  // When the embeddings setting changes, re-register the MCP entry so its
  // `env` field reflects the new value. Claude Code reads ~/.claude.json
  // at MCP-spawn time, so the user must restart Claude Code (or start a
  // new conversation) for the change to take effect.
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("claude-stats.recap.embeddings")) {
        ensureMcpServer(context);
        void vscode.window.showInformationMessage(
          t("extension:embeddings.settingChanged"),
        );
      }
    }),
  );
