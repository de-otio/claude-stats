...
function buildMcpEntry(): McpEntry {
  ...
  // Read the embeddings setting and forward it through the env field.
  // The MCP server reads CLAUDE_STATS_DEFAULT_EMBEDDINGS at handler time
  // and uses it as the default when the agent does not pass `embeddings`
  // explicitly. The value is constrained by the contributes-configuration
  // enum to "auto" | "off"; we still validate defensively below before
  // writing it into ~/.claude.json.
  const cfg = vscode.workspace.getConfiguration("claude-stats");
  const rawMode = cfg.get<string>("recap.embeddings", "auto");
  const embeddingsMode: "auto" | "off" =
    rawMode === "off" ? "off" : "auto";
  ...
  return {
    ...
    env: {
      CLAUDE_STATS_DEFAULT_EMBEDDINGS: embeddingsMode,
    },
  };
}
...
export function ensureMcpServer(_context: vscode.ExtensionContext): void {
  try {
    ...
    // Check if the entry is already up-to-date — including the env field, so
    // a settings change forces a re-write that Claude Code will pick up next
    // time it spawns the MCP server.
