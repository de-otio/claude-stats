...
export const MODEL_BYTES = 22_972_370;
 ...
 */
/**
 * File path of the bundled ONNX model inside a VSIX-style modelDir.
 * Layout matches what scripts/fetch-embedding-model.mjs writes and what
 * @huggingface/transformers expects when loading via env.localModelPath.
 */
export function bundledOnnxPath(modelDir: string): string {
  return path.join(
    modelDir,
    'Xenova',
    'all-MiniLM-L6-v2',
...
export async function createEmbeddingProvider(
  ...
  // ── Bundled layout (used by the VS Code extension) ────────────────────────
  //
  // When `modelDir` was supplied AND it contains the bundled HF tree at
  //   <modelDir>/Xenova/all-MiniLM-L6-v2/onnx/model_quantized.onnx
  // we treat it as the read-only VSIX bundle: verify the ONNX hash, then
  // load via @huggingface/transformers' env.localModelPath. This skips the
  // network entirely.
  //
  // SR-5 deviation: the VSIX directory is read-only on most installs, so
  // delete-on-mismatch is replaced with log-and-fall-through. Task 08
  // surfaces a loud warning at extension activation time.
  const bundledOnnx = bundledOnnxPath(modelDir);
  if (opts?.modelDir !== undefined && fs.existsSync(bundledOnnx)) {
    const ok = verifyModelHash(bundledOnnx);
    if (!ok) {
      console.error(
        `[recap/embeddings] Bundled model hash mismatch at ${bundledOnnx}. ` +
          `Falling back to lexical clustering. The extension activation ` +
          `integrity check will surface this as a warning.`,
      );
      return null;
    }
    return loadProvider(bundledOnnx, cacheDir, { localModelDir: modelDir });
  }

  // ── Legacy single-file layout (CLI download path) ─────────────────────────
  //
...
async function loadProvider(
  ...
  try {
    ...
    // ── Bundled mode: load from VSIX-supplied modelDir, no network ───────────
    //
    // When the VS Code extension supplies a bundled modelDir, configure
    // transformers.js to load from there and refuse remote downloads. The
    // hash check above guarantees the ONNX file matches MODEL_SHA256; the
    // tokenizer/config files ride the marketplace-signed VSIX boundary.
    if (loadOpts?.localModelDir !== undefined) {
      transformers.env.localModelPath = loadOpts.localModelDir;
