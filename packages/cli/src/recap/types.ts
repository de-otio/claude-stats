...
export interface DailyDigest {
  ...
  snapshotHash: string;
  /**
   * Which similarity rule actually ran for prompt-based clustering (rule 3
   * in cluster.ts). `"embeddings"` when an embedding provider was supplied
   * to buildDailyDigest; `"jaccard"` otherwise. Cached digests written
   * before this field existed are normalised to `"jaccard"` on read.
   */
  clusteringMethod: 'embeddings' | 'jaccard';
}
