/**
 * Query.teamProjectInsights — Pipeline Step 1: Verify team membership.
 ...
 *   Step 2: Query TeamStats, filter by projectId, aggregate, return ProjectInsights
 *
 * Args:
 *   teamId: ID!
...
export function request(ctx) {
  const { teamId, projectId, period } = ctx.args;
  if (!teamId) {
    util.error("teamId is required", "ValidationError");
  }
  if (!projectId) {
    util.error("projectId is required", "ValidationError");
  }
  if (!period) {
...
export function response(ctx) {
  ...
  // Stash caller's membership for share level context
