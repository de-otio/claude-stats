...
// Domain the deployed PreSignUp Lambda's SSM allowlist accepts. Required to
// run these integration tests; tests are skipped when unset.
const ALLOWED_DOMAIN = process.env.TEST_ALLOWED_EMAIL_DOMAIN;
const describeIfAllowed = ALLOWED_DOMAIN ? describe : describe.skip;

describeIfAllowed("Magic Link Authentication Flow", () => {
  describe("SignUp", () => {
    test("should create a new user with allowed domain email", async () => {
      const email = `test-${Date.now()}@${ALLOWED_DOMAIN}`;
  ...
  describe("Magic Link Challenge", () => {
    ...
    beforeAll(async () => {
      ...
      testEmail = `magic-link-test-${Date.now()}@${ALLOWED_DOMAIN}`;
    ...
    test("should rate-limit magic link requests", async () => {
      const limitTestEmail = `rate-limit-test-${Date.now()}@${ALLOWED_DOMAIN}`;
  ...
  describe("Error Handling", () => {
    test("should handle non-existent users gracefully", async () => {
      ...
      const nonExistentEmail = `nonexistent-${Date.now()}@${ALLOWED_DOMAIN}`;
